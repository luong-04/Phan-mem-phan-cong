...
<head>
    ...
    <title>Phần mềm Phân công Giám thị</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <nav class="bg-indigo-700 text-white p-4 shadow-md print-hidden">
        <div class="container mx-auto flex items-center">
            <h1 class="text-xl font-bold tracking-wide">Hệ thống Phân công Giám thị</h1>
        </div>
    </nav>

    <main class="container mx-auto px-4 py-8" x-data="proctorManager()">
        
        <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print-hidden">
            <div>
                <h2 class="text-3xl font-extrabold text-gray-800 tracking-tight">Quản lý Lịch thi</h2>
        ...
        <div x-show="activeTab === 'setup'" style="display: none;" class="print-hidden">
            <form action="{{ route('proctors.assign') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                @csrf
    ...
    <script>
        ...
        function fetchExamDates() {
            ...
            axios.get(`/proctors/${examId}/history?_t=${timestamp}`).then(response => {
        ...
        function renderTableForSelectedDate() {
            ...
            dailyData.forEach((item, index) => {
                // Đã xử lý: Xóa chữ "Phòng " và xóa số "0" dư thừa ở trước (VD: Phòng 07 -> 7)
                let roomName = item.room !== "(Trống)" ? item.room.replace(/Phòng\s*/gi, '').replace(/^0+(?=\d)/, '') : "";
        ...
        function deleteExam() {
            ...
            if(confirm("⚠️ CẢNH BÁO: Bạn có chắc chắn muốn xóa toàn bộ lịch thi này?\nHành động này không thể hoàn tác!")) {
                axios.delete(`/proctors/${examId}`, {
                    headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
                }).then(res => {
                    alert("Đã xóa lịch thi thành công!");
                    // Đã xử lý: Thêm ?auto_load_exam= rỗng để phần mềm tự động bật lại tab Lịch Sử thay vì Setup
                    window.location.href = "/?auto_load_exam="; 
                }).catch(err => {
