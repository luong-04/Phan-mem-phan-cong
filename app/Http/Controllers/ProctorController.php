...
class ProctorController extends Controller
{
    ...
    public function assign(Request $request)
    {
        ...
        return redirect('/?auto_load_exam=' . $exam->id)
        ->with('success', 'Đã phân công thành công kỳ thi: ' . $exam->name);
    }

    public function showHistory($id)
    {
        // Nhận trực tiếp $id từ URL (ví dụ: /proctors/1/history)
        $exam = Exam::find($id);
        
        // Trả về lỗi 404 nếu không tìm thấy để Axios bắt lỗi chính xác
        if (!$exam) return response()->json(['error' => 'Không tìm thấy kỳ thi'], 404);
